...
class parts_list():
	...
	def create_parts_list(self,conn,rma,service_fee,vnd=23700,folder_name='exports'):
		...
		# part_list_final = self.part_list_final(rma,conn)
			#--------------xlwings--------------------
			wb = xw.Book('files\\Baogia_Template.xlsx')
			...
			price_type = str(input('Price for ffvn or DEALER: ') or 'Dealer')
...
class technical_report():
	...
	def signatures(self,tp,signatures_folder='signatures'):
		...
		if tp.range('F50').value == 'Nguyễn Khắc Thắng':
			tp.pictures.add(os.path.join(path,signatures_folder,'Thang.png'),name = 'Thang',top = tp.range('G44').offset(1,0).top+7,left = tp.range('G44').offset(1,0).left-20)
		if tp.range('F50').value == 'Nguyễn Thái Nguyên':
			tp.pictures.add(os.path.join(path,signatures_folder,'Nguyen.png'),name = 'Nguyen',top = tp.range('G44').offset(1,0).top+7,left = tp.range('G44').offset(1,0).left-20)
	...
	def report(self,conn,info,part_list,report_num,folder='files'):
		...
		# tp.range('D21').value = str(add_info['leader'][0])
		tp.range('D21').value = str("'") + str(add_info['date_installed'][0])
		tp.range('D22').value = str(add_info['scope_count'][0])
		tp.range('D23').value = str("'") + str(add_info['last_repair'][0])
		tp.range('D24').value = str("'") + str(add_info['receive'][0])
		...
		if ('XL' in model) or('VP' in model):
			...
			tp.pictures.add(os.path.join(path,'files',"eb.jpg"),name='Model',top = top_model,left = left_model)
		elif ('201' in model) or ('250' in model):
			tp.pictures.add(os.path.join(path,'files',"250.jpg"),name='Model',top = top_model,left = left_model)
		elif ('450' in model) or ('470' in model):
			tp.pictures.add(os.path.join(path,'files',"400.jpg"),name='Model',top = top_model,left = left_model)
		elif ('530' in model) or ('580' in model) or ('590' in model) or ('600' in model):
			tp.pictures.add(os.path.join(path,'files',"500.jpg"),name='Model',top = top_model,left = left_model)
		elif ('720' in model) or ('740' in model) or ('760' in model):
			tp.pictures.add(os.path.join(path,'files',"700.jpg"),name='Model',top = top_model,left = left_model)
		...
		for i in range(len(part_list)):
			tp.range('B'+str(i+33)).value = "'- " + str(part_list['Vie'][i])
			if str(part_list['Vie'][i]) =='None':
				tp.range('F'+str(i+33)).value = str(part_list['PART_DESCRIPTION'][i])
		
		#format evulation
		if len(ins_code)>1:   
			for i in range(len(ins_code)-1):
				tp.range('30:30').insert()
				tp.range('B30:C30').merge()
				tp.range('D30:E30').merge()
				tp.range('F30:G30').merge()
				tp.range('H30:J30').merge()

			tp.range('A29:J31').copy()
			tp.range('A32:J'+str(len(ins_code)+30)).paste('formats')   

		#add code
		for i in range(len(ins_code)):
			tp.range('A'+str(i+30)).offset(1,0).value = i+1
			tp.range('B'+str(i+30)).offset(1,0).value = ins_code['F_DESCRIPTION'][i]
			tp.range('D'+str(i+30)).offset(1,0).value = ins_code['P_DESCRIPTION'][i]
			tp.range('F'+str(i+30)).offset(1,0).value = ins_code['C/D-Detail'][i]
			tp.name = 'BCGDKT'
		return wb
