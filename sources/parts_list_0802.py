...
class parts_list():
	...
	def part_list_final(self,rma,conn):
		...
		q=f'''
			...
			ORDER BY priority, p.part_description
...
class technical_report():
	...
	def report(self,conn,info,part_list,report_num,folder='files'):
		...
		for i in range(len(part_list)):
			tp.range('B'+str(i+34)).offset(1,0).value = "'- " + str(part_list['Vie_GD'][i])
			if str(part_list['Vie_GD'][i]) =='None': # add english name in F column
