...
class technical_report():
	...
	def create_qr_image(self,info,image_folder='images'):
		Logo_link = 'files\\Fujifilm10nam2.png'
	...
	def report(self,conn,info,part_list,report_num,folder='files'):
		...
		tp.range('A8').value = f"Số:{today.strftime('%y')}{today.strftime('%m')}{report_num}/BCGĐKT"
		...
		for i in range(len(part_list)-1):
			tp.range('35:35').offset(1,0).insert()
		for i in range(len(part_list)):
		...
		#format evulation
		if len(ins_code)>1:
			for i in range(len(ins_code)-1,0):
				tp.range('31:31')insert()
				tp.range('B31:C31').merge()
				tp.range('D31:E31').merge()
				tp.range('F31:G31').merge()
				tp.range('H31:J31').merge()

			tp.range('A30:J30').copy()
			# tp.range('A32:J'+str(len(ins_code)+29)).offset(1,0).paste('formats')   
			tp.range('A31:J'+str(len(ins_code)+30)).paste('formats')   

		#add code
		for i in range(len(ins_code)):
			tp.range('A'+str(i+30)).offset(0,0).value = i+1
			tp.range('B'+str(i+30)).offset(0,0).value = ins_code['F_DESCRIPTION'][i]
			tp.range('D'+str(i+30)).offset(0,0).value = ins_code['P_DESCRIPTION'][i]
			tp.range('F'+str(i+30)).offset(0,0).value = ins_code['C/D-Detail'][i]
			tp.name = 'BCGDKT'
		return wb
	def save_and_close(self,wb,i_report,model,sn,folder_name='files'):
		model = model.replace('-','')
		model = model.replace(' V2','')
		file_name = f"FFVN-Endo-GDKT-{dt.now().strftime('%y')}{dt.now().strftime('%m')}{i_report}-{model.replace('/','')},{sn}.xlsx"
